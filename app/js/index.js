...
var $items = $('.article'); // Анимация статей по вьюпорту
...
$(window).on('load', function () {
  if ($('html').hasClass('page-article')) {
    var $imgs = $('.article-wrapper img');
    _gsap.TweenMax.set([$imgs], { transition: '1s ease' });
...
var _scrollDirection = __webpack_require__(51);
...
$(document).ready(function () {
  var isMainPage = $('html').hasClass('page-main');
  var isCatalogPage = $('html').hasClass('page-catalog');
  ...
  if (isMainPage) {
    ...
    var downHeader = function downHeader() {
      return $header.addClass('is-active');
    };
    var upHeader = function upHeader() {
    ...
    $(window).on('scroll', function () {
      ...
      if ((0, _scrollDirection2.default)() === -1 && window.pageYOffset > 0) {
        downHeader();
      } else if ((0, _scrollDirection2.default)() === 1 && window.pageYOffset > 0) {
        upHeader();
      }
    });
  }
  ...
  // Поведение на странице каталога
      ...
      var _$header = $('.main-wrapper .header');

      var _downHeader = function _downHeader() {
        return _$header.addClass('is-active');
      };
      var _upHeader = function _upHeader() {
        return _$header.removeClass('is-active');
      };
...
exports.default = getScrollDirection;
...
document.querySelector('html').classList.add('page-main');
...
// Base
var $btn = $('.hero__toggle'); // Слайдер фоновых изображений на первом экране

var $cover = $('.hero-cover');
var $images = $('.hero-image');
var $icons = $('.hero__icon-wrap');
var $contentBlocks = $('.hero__content-block');

var activeIndex = 0;
var activeImg = $images[activeIndex];
var activeIcon = $icons[activeIndex];
var activeContent = $contentBlocks[activeIndex];
...
var timer = void 0;
var timerInterval = 16000;

// Колбэки
var unsetCover = function unsetCover() {
  $cover.hide();
  _gsap.TweenMax.set($cover[0], { clearProps: 'transform' });
  $cover.show();
  notAnimated = true;
};
var onCover = function onCover() {
  ...
  _gsap.TweenMax.set(activeImg, { x: '5%' });
  _gsap.TweenMax.set(activeIcon, { scale: 0 });

  // Белый / Темный режимы
  $('#hero, .header--mob').removeClass('light dark');
  var color = activeImg.dataset.color;
  ...
  $(activeIcon).show();
  $(activeContent).show();
  _gsap.TweenMax.to(activeImg, 0.8, { x: '0%' });
  _gsap.TweenMax.to(activeIcon, 0.6, { scale: 1, ease: _gsap.Back.easeOut.config(2) });
};
var onBtnClick = function onBtnClick() {
  if (notAnimated) {
    ...
    tl.to(activeIcon, 0.6, { scale: 0 }).fromTo(activeImg, 0.8, { x: '0%' }, { x: '-5%' }, '-=0.5').to($cover[0], 0.6, {
      x: '0%',
      onComplete: onCover
    }, '-=0.8').to($cover[0], 0.6, {
      x: '-100%',
...
var setAutoSlide = function setAutoSlide() {
  timer = setInterval(onBtnClick, timerInterval);
};
...
var _wheelDirection = __webpack_require__(82);

var _wheelDirection2 = _interopRequireDefault(_wheelDirection);
...
var _ScrollToPlugin = __webpack_require__(84);

var _ScrollToPlugin2 = _interopRequireDefault(_ScrollToPlugin);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } } // Поэкранные слайды
...
var Slide = function Slide(el) {
  ...
  this.hide = function () {
    _this.$el.removeClass('is-visible');
  };

  this.goTop = function (callback) {
    var $prev = _this.$el.prev();

    if ($prev.length && !_gsap.TweenMax.isTweening(window)) {
      new _gsap.TimelineMax().to(window, _this.duration, {
  ...
  this.goBtm = function (callback) {
    ...
    if ($next.length && !_gsap.TweenMax.isTweening(window)) {
      new _gsap.TimelineMax().to(window, _this.duration, {
        scrollTo: scrTop,
        ease: _this.easing,
        onComplete: callback
  ...
  // Параметры анимации
  this.easing = _gsap.Expo.easeInOut;
  this.duration = 1.2;
...
var _doc = (__WEBPACK_IMPORTED_MODULE_0__TweenLite_js__["k" /* _gsScope */].document || {}).documentElement,
		_window = __WEBPACK_IMPORTED_MODULE_0__TweenLite_js__["k" /* _gsScope */],
		...
		ScrollToPlugin = __WEBPACK_IMPORTED_MODULE_0__TweenLite_js__["k" /* _gsScope */]._gsDefine.plugin({
			propName: "scrollTo",
			API: 2,
			global: true,
			version:"1.9.1",
...
$(document).ready(function () {
  if ($('html').hasClass('page-main')) {
    $('.products__content').clone().addClass('products__content--is-clone').insertAfter('.products__content:last');

    var gr = new _Grabber2.default($('.products__content'));
  }
});
